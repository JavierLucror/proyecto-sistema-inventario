...
constructor(private crudProductoService: CrudProductoService, private crudPedidoService: CrudPedidoService, private ruteador: Router) {}
...
sumarTotal()  {
  ...
  document.getElementById('totalNumPedido')!.innerHTML = str_total + "€";
}
...
sumarCantidadTotal()  {
  ...
  document.getElementById('cantidadNumPedido')!.innerHTML = str_cantidad;
}

//Datos del proveedor y fecha actual
recogerDatos()  {
  let proveedorActual: string = localStorage.getItem('item')?.toString() || '';
  let fechaActual = formatDate(Date.now(),'dd-MM-yyyy','en-US').toString();
   
  document.getElementById('usuario')!.innerHTML = proveedorActual;
...
async confirmarPedido()  {
  ...
  let proveedorActual: string = this.proveedor;
  let fechaActual = formatDate(Date.now(),'dd-MM-yyyy','en-US').toString();
  let total: string = parseInt(document.getElementById('totalNumPedido')!.innerHTML).toFixed(2);
  let totalActual: number = parseInt(total);
  let datosJSON = JSON.stringify(this.productos);

  if(proveedorActual != '')  {
    await Swal.fire({
      title: "¿Confirmar registro de pedido?", 
      showCancelButton: true, 
      confirmButtonText: "Confirmar"
    }).then((result) => {
      if(result)
    ...
    if(confirma)  {
      this.crudPedidoService.registrarPedido(usuarioActual, proveedorActual, fechaActual, totalActual, datosJSON).subscribe(()=>{});
      setTimeout(() => {
        window.location.reload();
      }, 500);
      this.ruteador.navigateByUrl('/pedidos');
    }

  }else {
    await Swal.fire({
      title: "Indique el proveedor para el pedido", 
      showCancelButton: false, 
      confirmButtonText: "Vale"
    }).then((result) => {
      if(result) {
